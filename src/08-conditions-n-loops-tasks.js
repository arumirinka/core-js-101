...
function doRectanglesOverlap(rect1, rect2) {
  return ((rect1.height + rect1.top) > rect2.top) && ((rect1.width + rect1.left) > rect2.left);
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${first}${Math.min(a, b)}, ${Math.max(a, b)}${last}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 1; i <= cardNum.length; i += 1) {
    let digit = +cardNum[cardNum.length - i];
    if (i % 2 === 0) digit *= 2;
    if (digit > 9) digit -= 9;
    sum += digit;
  }
...
function getDigitalRoot(num) {
  const result = num.toString().split('').reduce((acc, el) => acc + Number(el), 0);
...
function getCommonDirectoryPath(pathes) {
  const shortest = pathes.sort((a, b) => a.length < b.length).pop().split('/');
  const res = [];
  for (let i = 0; i < shortest.length; i += 1) {
    if (pathes.every((path) => path.split('/')[i] === shortest[i])) res.push(shortest[i]);
    else break;
  }
